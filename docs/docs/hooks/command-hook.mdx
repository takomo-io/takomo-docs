...
| TKM_HOOK\_\<hook-name> | Values returned from previous hooks are exposed in environment variables where the \<hook-name> placeholder is replaced with the hook's name. Hooks whose name has unsafe characters not compatible with a pattern `/^[a-zA-Z_]+[a-zA-Z0-9_]*$/` are not exposed and a warning is logged instead. |
